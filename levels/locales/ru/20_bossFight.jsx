 ...
 *****************
 *
 * НИ ШАГУ ДАЛЬШЕ, Д-Р EVAL!!!!
 * ЖИВЫМ ТЫ ОТСЮДА НЕ ВЫЙДЕШЬ!!!!
 * ПОРА ТЕБЕ УВИДЕТЬ МОЙ ИСТИННЫЙ ОБЛИК!!!!
 * УЗРИ МОЙ РОБОТИЧЕСКИЙ ГНЕВ!!!!!
 */
...
function validateLevel(map) {
    // вызывается при старте уровня и при каждом callback
    ...
    if (map.countObjects('theAlgorithm') > 0 && map.countObjects('boss') > 0) {
        throw "Алгоритм может выронить только босс!";
    }

    // вызывается только при старте уровня
...
function onExit(map) {
    if (!map.getPlayer().hasItem('theAlgorithm')) {
        map.writeStatus("Вы должны вернуть Алгоритм!!");
        return false;
