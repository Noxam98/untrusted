 ...
 *************
 *
 * Пора выпустить убийственные лазеры! Каждый лазер убьёт вас,
 * если у вас не тот цвет. Жаль только, что не видно, какому
 * лазеру какой цвет соответствует!
 */
...
#START_OF_START_LEVEL#
    ...
    function createLaser(centerX, centerY, angleInDegrees, length, color) {
        ...
        // map.createLine() создаёт линию с эффектом при
        // прохождении игрока, но саму линию не показывает
        map.createLine([x1, y1], [x2, y2], function (player) {
            if (player.getColor() != color) {
                player.killedBy('лазер (' + color + ')');
            }
        });
